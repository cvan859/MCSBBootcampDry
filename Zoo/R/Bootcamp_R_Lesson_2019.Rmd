...
#Step 4: Finally, we can compute the Pearson correlation (p-value is cor.test) 
...
#specific to dplyr: %>% syntax will take the data output from the previous step and perform the task listed after the arrow on that data 
#(see Linux pipes concept) don't need to save intermediate files
...
#It's basically a better Pivot Table
...
# without position='dodge', ggplot will do a stacked bar graph
```
...
#since we have multiple columns to add to the metadata, we will want to ('c() makes everything into a vector)
...
#can run each piece before the plus to see sequential commands
```
...
pheatmap(expression_matrix[genes_of_interest,],
         ...
         show_colnames = FALSE)
#annotation_col adds clusters by orig.ident and res.0.4 (see top two rows in heatmap)
```
